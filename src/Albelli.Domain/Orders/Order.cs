...
namespace Albelli.Domain.Orders
{
    public class Order : Entity
    {
        ...
        public MoneyValue _value { get; private set; }
        ...
        public double _requiredBinWidth { get; private set; }
            ...
            )
        {
            ...
            this.CalculateRequiredBinWidth(orderProductsData);
        ...
        private void CalculateRequiredBinWidth(List<OrderProductData> orderProductsData)
        {
            double requiredBinWidth = 0.0;
            foreach (var item in orderProductsData)
            {
                if (item.ProductType == ProductType.Mug)
                {
                    requiredBinWidth += (item.Quantity / 4) * item.PackageWidth + (item.Quantity % 4) * item.PackageWidth;
                }
                else
                {
                    requiredBinWidth += item.Quantity * item.PackageWidth;
                }

            }

            _requiredBinWidth = requiredBinWidth;
        }
    }
}